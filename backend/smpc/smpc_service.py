...
class SMPCProtocol:
    ...
    def _deserialize_result(self, result: bytes) -> Any:
    ...
    def compute_multiplication(self, shares_dict1: Dict[str, bytes],
        ...
        try:
            ...
            for party in shares_dict1.keys():
                decrypted1 = self.cipher.decrypt(shares_dict1[party])
                decrypted2 = self.cipher.decrypt(shares_dict2[party])
                shares1.append(self._deserialize_result(decrypted1))
                shares2.append(self._deserialize_result(decrypted2))
