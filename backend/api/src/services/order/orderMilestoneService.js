...
export class OrderMilestoneService {
    ...
    return measureExecution('OrderMilestoneService.updateMilestone', async () => {
      ...
      if (milestone === 'Delivered') {
        throw new DomainError(400, {
          error: 'Cannot set Delivered milestone directly. Use /verify-delivery endpoint to confirm delivery.'
        });
      }
      ...
      if (!timelineEntry)
        throw new DomainError(400, {
          error: `Milestone "${milestone}" is not part of this order's timeline.`
        });

      if (timelineEntry.completed) {
        throw new DomainError(409, {
          error: `Milestone "${milestone}" has already been completed.`
        });
      }
  ...
  async verifyDelivery({ orderId, otp, driverId }, userClient) {
    return measureExecution('OrderMilestoneService.verifyDelivery', async () => {
      ...
      const isMatch = verifyDeliveryOtpHash(otp, otpRecord);
      if (!isMatch) {
        ...
        logger.warn(
          `[OTP] Failed verification attempt for order ${orderId} by driver ${driverId}. ${remaining} attempts remaining.`
        );
      ...
      const { data: tripData, error: rpcErr } = await this.orderRepository.executeRpc(
        'complete_trip_tx',
        {
          p_order_id: orderId,
      ...
      if (rpcErr) {
        ...
        throw new DomainError(500, {
          error: 'Failed to complete trip and release payment.',
          details: rpcErr.message
        });
      }

      const { data: verifiedOrder, error: verifyErr } = await this.orderRepository.findOrderById(
        orderId,
      ...
      if (releaseTxHash || escrowAlreadyReleased) {
        ...
        if (driverIdVal) {
          const { error: walletErr } = await this.orderRepository.updateWalletTransaction(driverIdVal, displayId, {
